...
class FrontierCrudViewsGenerator < Frontier::Generator
  ...
  def scaffold
    unless model.skip_ui?
      ...
      generate_feature_path("index_spec.rb", "#{model.name.as_collection}_index_spec.rb") if model.show_index?
      ...
      if model.attributes.any?(&:sortable?)
        generate_feature_path("sort_index_spec.rb", "#{model.name.as_collection}_sort_index_spec.rb") if model.show_index?
      end
    end
  end
